...
SAMPLELIST=${1:-/projects/f_mlp195/kyraf/croaker/lcwgs/sample_lists/Sample_List_11.tsv} #Path to a list of prefixes of the raw fastq files. It should be a subset of the the 1st column of the sample table. An example of such a sample list is /workdir/cod/greenland-cod/sample_lists/sample_list_pe_1.tsv
SAMPLETABLE=${2:-/projects/f_mlp195/kyraf/croaker/lcwgs/sample_lists/Sample_Table_11.tsv} #Path to a sample table where the 1st column is the prefix of the raw fastq files. The 4th column is the sample ID, the 2nd column is the lane number, and the 3rd column is sequence ID. The combination of these three columns have to be unique. The 6th column should be data type, which is either pe or se. An example of such a sample table is: /workdir/cod/greenland-cod/sample_lists/sample_table.tsv
FASTQDIR=${3:-/projects/f_mlp195/kyraf/croaker/lcwgs/qual_filtered_q30_trim/} #Path to the directory where fastq file are stored. An example for the quality-filtered Greenland pe data is: /workdir/cod/greenland-cod/qual_filtered/
BASEDIR=${4:-/projects/f_mlp195/kyraf/croaker/lcwgs/} #Path to the base directory where adapter clipped fastq file are stored in a subdirectory titled "adapter_clipped" and into which output files will be written to separate subdirectories. An example for the Greenland cod data is: /workdir/cod/greenland-cod/
FASTQSUFFIX1=${5:-_adapter_clipped_qual_filtered_f_paired.fastq.gz} #Suffix to fastq files. Use forward reads with paired-end data. An example for the quality-filtered Greenland paired-end data is: _adapter_clipped_qual_filtered_f_paired.fastq.gz
FASTQSUFFIX2=${6:-_adapter_clipped_qual_filtered_r_paired.fastq.gz} #Suffix to fastq files. Use reverse reads with paired-end data. An example for the quality-filtered Greenland paired-end data is: _adapter_clipped_qual_filtered_r_paired.fastq.gz
MAPPINGPRESET=${7:-sensitive-local} #The pre-set option to use for mapping in bowtie2 (very-sensitive for end-to-end (global) mapping [typically used when we have a full genome reference], very-sensitive-local for partial read mapping that allows soft-clipping [typically used when mapping genomic reads to a transcriptome]
...
THREADS=${10:-64} #Number of threads to use. Default is 8
MINQ=${11:-0} #Minimum mapping quality filter. Default is 0 (no filter)
SAMTOOLS=${12:-/home/ksf63/samtools-1.17/samtools}
...
for SAMPLEFILE in `cat $SAMPLELIST`; do
	...
	SAMPLEBAM=$BASEDIR'bam1s_local_sort/'$SAMPLE_SEQ_ID
