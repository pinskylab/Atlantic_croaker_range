...
cov_matrix <- as.matrix(read.table("pcangsd_bam_list_realigned_mindp132_maxdp4000_minind0.cov"))
#Matrix is in order 1-400 on each side
ind_label <- seq(1:400)
pop_label <- read.csv("pop_labels.csv", header=FALSE)
pop_label <- as.vector(pop_label[,1])
pop_label <- replace(pop_label, pop_label=="NJ", "DE")
x_axis <- 1
y_axis <- 2



PCA <- function(cov_matrix, ind_label, pop_label, x_axis, y_axis, show.point=T, show.label=F, show.ellipse=T, show.line=T, alpha=0)
{
...
e$population <- factor(e$population, levels=c("DE", "MD", "VA", "NC", "OB", "LB", "SC", "GA", "FL"))
e[173, 2] <- "DE"
e[185:188, 2] <- "DE"
...
PCA_plot<-ggplot(data=e[,],aes(x=e[,x_axis+2], y=e[,y_axis+2], color=population)) +
  geom_enterotype(alpha=0.75, show.point=show.point, show.label=F, show.ellipse=show.ellipse, show.line=show.line, size=2) +
  theme_half_open() +
  scale_color_manual(values=c("orchid4", "skyblue1", "sienna2", "slategrey", "gold", "saddlebrown", "red3", "pink1", "blue4" )) +
  #scale_color_brewer(name=waiver(), palette="Set1") +
  xlab(paste0("PC", x_axis, "(",round(x_variance,2),"%)")) +
  ylab(paste0("PC", y_axis ,"(",round(y_variance,2),"%)"))
print(PCA_plot)

PCA_plot<-ggplot(data=e,aes(x=PC1, y=PC2, color=population)) +
...
ggplot(data=e,aes(x=PC1, y=PC2, color=population)) +
...
DAPC <- function (n=50, x_axis, y_axis, show.point=T, show.label=T, show.ellipse=T, show.line=T, alpha=0) {
  ...
  DAPC_plot<- ggplot(dataset, aes(dataset[,1+x_axis], dataset[,1+y_axis], color= group, label=group, shape=group)) +
    geom_enterotype(show.point=show.point, show.label=F, show.ellipse=show.ellipse, show.line=show.line, alpha=alpha) +
    scale_shape_manual(values = c(rep(c(15,16,17,18),7), 15, 16)) +
    theme_cowplot() +
