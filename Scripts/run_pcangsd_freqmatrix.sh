...
BASEDIR=${1:-/projects/f_mlp195/kyraf/croaker/lcwgs/} # Path to the base directory where adapter clipped fastq file are stored in a subdirectory titled "adapter_clipped" and into which output files will be written to separate subdirectories. An example for the Greenland cod data is: /workdir/cod/greenland-cod/
BEAGLE=${2:-/projects/f_mlp195/kyraf/croaker/lcwgs/angsd/bam_list_realigned_mindp132_maxdp4000_minind0_minmaf0.05.beagle.gz} # Path to the beagle formatted genotype likelihood file
...
if [ $ANALYSIS = pca ]; then
	pcangsd -beagle $BEAGLE -minMaf 0.05 -threads 16 -indf_save -o $BASEDIR'angsd/pcangsd_minmaf0.05_'$PREFIX
