...
#Following tutorial by Brenna Forester (https://bookdown.org/hhwagner1/LandGenCourse_book/WE_11.html)

##Individual frequencies using all filters applied dataset (~23K SNPs)

#See rda_analysis.R script for how genetic data was loaded and formatted
...
length(which(lfmm_ocean_filters.qv < 0.05)) #351
...
length(intersect(lfmm_ocean_filters_q0.1, outliers_ocean$Loci)) #381 in both RDA and LFMM
...
length(which(lfmm_ocean_filters_temp.qv < 0.05)) #263
...
lfmm_ocean_filters_q0.05_temp <- colnames(indfreq_filters)[which(lfmm_ocean_filters_temp.qv < 0.05)]

length(intersect(lfmm_ocean_filters_q0.05_temp, outliers_ocean$Loci)) #263 candidates in RDA and LFMM
...
length(which(lfmm_ocean_filters_oxygen.qv < 0.05)) #3
...
lfmm_ocean_filters_q0.05_oxygen <- colnames(indfreq_filters)[which(lfmm_ocean_filters_oxygen.qv < 0.05)]

length(intersect(lfmm_ocean_filters_q0.05_oxygen, outliers_ocean$Loci)) #3 candidates in both RDA and LFMM
...
length(which(lfmm_ocean_filters_salinity.qv < 0.05)) #0
...
length(which(lfmm_ocean_filters_depth.qv < 0.05)) #0
...
length(which(lfmm_ocean_filters_lat.qv < 0.05)) #407
...
lfmm_ocean_filters_q0.05_lat <- colnames(indfreq_filters)[which(lfmm_ocean_filters_lat.qv < 0.05)]

length(intersect(lfmm_ocean_filters_q0.05_lat, outliers_ocean$Loci)) #407 in both RDA and LFMM
