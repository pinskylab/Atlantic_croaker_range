...
k2_pc <- as.data.frame(pcadmix.id.order$q1)
k2_pc$q2 <- pcadmix.id.order$q2
colnames(k2_pc)<- c("q1", "q2")
k2_pc$q1 <- as.numeric(k2_pc$q1)
k2_pc$q2 <- as.numeric(k2_pc$q2)

meta.data <- data.frame(loc=as.vector(pcadmix.id.order$pop))
meta.data$loc <- as.character(meta.data$loc)
meta.data$loc[1:5] <- "DE"

q2_pc <- list(k2_pc)
admix <- plotQ(as.qlist(q2_pc), imgoutput = "sep", returnplot = TRUE, returndata = TRUE, exportpath=getwd(), dpi=1000,
      clustercol = c("lemonchiffon", "mediumorchid4"),
