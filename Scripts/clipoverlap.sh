...
BAMLIST=${1:-/projects/f_mlp195/kyraf/croaker/lcwgs/sample_lists/bam_list_merged.tsv} # Path to a list of merged, deduplicated, and overlap clipped bam files. Full paths should be included. An example of such a bam list is /workdir/cod/greenland-cod/sample_lists/bam_list_1.tsv
SAMPLETABLE=${2:-/projects/f_mlp195/kyraf/croaker/lcwgs/sample_lists/sample_table_merged.tsv} # Path to a sample table where the 1st column is the prefix of the MERGED bam files. The 4th column is the sample ID, the 2nd column is the lane number, and the 3rd column is sequence ID. The 5th column is population name and 6th column is the data type. An example of such a sample table is: /workdir/cod/greenland-cod/sample_lists/sample_table_merged.tsv
PICARD=${3:-/home/ksf63/picard.jar} # Path to picard tools
BAMUTIL=${4:-/home/ksf63/bamUtil/bam} # Path to bamUtil
...
for SAMPLEBAM in `cat $BAMLIST`; do
	...
	DATATYPE=`grep -P "${SAMPLESEQID}\t" $SAMPLETABLE | cut -f 6`
		...
		$BAMUTIL clipOverlap --in $SAMPLEBAM --out $SAMPLEPREFIX'_dedup_overlapclipped.bam' --stats
	#fi
